                       ...
                       LogEvent::ptr event)
{
    for (auto& i : m_items)
    {
        switch (i.first)
        {
            ...
            case 'r': // 线程启动至今的秒数
            {
                os << gude::time_difference(event->getElapse());
                break;
            }
            case 'c': // 日志名称
            {
                       ...
                       LogEvent::ptr event)
{
    for (auto& i : m_items)
    {
        switch (i.first)
        {
            ...
            case 'r': // 线程启动至今的秒数
            {
                str.append(gude::time_difference(event->getElapse()));
                break;
            }
            case 'c': // 日志名称
            {
...
LoggerManager::LoggerManager()
{
    ...
    static gude::Config<std::set<LogDefine> >::ptr g_log_config =
        gude::ConfigMgr::GetInstance()->get("logs", std::set<LogDefine>(), "logs config");
