...
#define CalculateElapse() \
    gude::ProcessMgr::GetInstance()->getParentStartTime() == 0 ? 0 : \
        (time(0) - gude::ProcessMgr::GetInstance()->getParentStartTime())

// 使用流文本的方式输出日志(同步, 异步, udp)
#define log_stream(level) \
    if (level >= logger->getLevel()) \
        gude::LogEventWrap(gude::LogEvent::ptr(new gude::LogEvent(__FILE__, \
            __LINE__, CalculateElapse(), gude::Thread::getId(), gude::Fiber::getId(), \
            time(0), gude::Thread::getName(), logger, level))).getSS()

// 使用流文本的方式输出日志(同步, 异步, udp)
#define log_debug    log_stream(gude::LogLevel::DEBUG)
#define log_info     log_stream(gude::LogLevel::INFO)
#define log_warn     log_stream(gude::LogLevel::WARN)
#define log_error    log_stream(gude::LogLevel::ERROR)
#define log_fatal    log_stream(gude::LogLevel::FATAL)
...
#define log_fmt(level, fmt, ...) \
    if (level >= logger->getLevel()) \
        gude::LogEventWrap(gude::LogEvent::ptr(new gude::LogEvent(__FILE__, \
            __LINE__, CalculateElapse(), gude::Thread::getId(), gude::Fiber::getId(), time(0), \
            gude::Thread::getName(), logger, level))).getEvent()->format(fmt, __VA_ARGS__)

// 使用格式化文本的方式输出日志(同步, 异步, udp)
#define log_fmt_debug(fmt, ...)    log_fmt(gude::LogLevel::DEBUG, fmt, __VA_ARGS__)
#define log_fmt_info(fmt,  ...)    log_fmt(gude::LogLevel::INFO,  fmt, __VA_ARGS__)
#define log_fmt_warn(fmt,  ...)    log_fmt(gude::LogLevel::WARN,  fmt, __VA_ARGS__)
#define log_fmt_error(fmt, ...)    log_fmt(gude::LogLevel::ERROR, fmt, __VA_ARGS__)
#define log_fmt_fatal(fmt, ...)    log_fmt(gude::LogLevel::FATAL, fmt, __VA_ARGS__)


// 获取名称为name的日志器
#define NameLogger(name) static gude::Logger::ptr logger = gude::LoggerMgr::GetInstance()->get(name);
// 获取root日志器(应用程序使用)
...
// 日志器管理类单例
typedef gude::Singleton<LoggerManager> LoggerMgr;
