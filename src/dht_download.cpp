...
DhtDownload::DhtDownload(bifang::Address::ptr addr, const std::string& node_id,
...
{
    m_sock = bifang::Socket::createTCP(addr);
}

std::string DhtDownload::dump(const std::string& str)
{
    std::stringstream ss;
    size_t i = 0;
    while (i < str.size())
...
bool DhtDownload::download()
{
#define COMMON_PART   "hash(" << hash << "), address[" << *m_addr << "], "

    std::string hash = bifang::StringUtil::toHexString(m_info_hash);

    char* buf = new char[DOWNLOAD_BUF_LEN];
    ...
    int len = m_sock->recv(buf, DOWNLOAD_BUF_LEN);
    if (len < 68)
    {
    ...
    ext_handshake_message_size = bifang::littleByteSwap(ext_handshake_message_size);
    ...
    while (1)
    {
        int cur_len = m_sock->recv(buf + len, DOWNLOAD_BUF_LEN - len);
        if (cur_len <= 0)
            break;
        len += cur_len;
        if (len >= DOWNLOAD_BUF_LEN)
            break;
    }
    ...
    while (metadata_size > 0)
    {
        ...
        get_metadata_message_size = bifang::littleByteSwap(get_metadata_message_size);
