...
static Config<std::string>::ptr g_pid_file =
    ConfigMgr::GetInstance()->get("system.pid_file", std::string("dht_search.pid"), "system pid file config");
...
{
    IOManager* accept = gude::WorkerMgr::GetInstance()->getAsIOManager("accept").get();
    if (!accept)
    {
        log_error << "accept not exists";
        _exit(0);
    }
    gude::IOManager* process = gude::WorkerMgr::GetInstance()->getAsIOManager("process").get();
    if (!process)
    {
    ...
    IPv4Address::ptr addr(new IPv4Address(INADDR_ANY, 6881));
    dht::DhtSearchServer::ptr server(new dht::DhtSearchServer(accept, process));
    if (!server->bind(addr))
    {
